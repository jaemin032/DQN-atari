...
class Agent_DQN(object):
	...
	def get_path(self):
		...
		########################
		directory = './efficientnetb0_dense_layer_model'
		try:
	...
	def build_model(self):
		...
		with tf.compat.v1.variable_scope('eval_net'):
			print('self s shape')
			print(self.s.shape)
			eval_net = tf.keras.applications.EfficientNetB0(
				include_top=False, weights=None, input_tensor=self.s,
				pooling="max"
			)
			print(eval_net.summary())
			eval_net.trainable = True
			e_flat = tf.compat.v1.layers.flatten(eval_net.output)
			e_dense1 = tf.compat.v1.layers.dense(
				inputs=e_flat,
				units=64,
			...
			# model = Model(inputs=self.s, outputs=self.q_new)
			# print(model.summary())
			#
			# print(self.q_new.summary())


			# e_conv1 = tf.compat.v1.layers.conv2d(
			# 		inputs=self.s,
			# 		filters=32,
			# 		kernel_size=[8, 8],
			# 		strides=(4, 4),
			...
			# 		kernel_initializer=w_initializer,
			# 		name='e_conv1')
			# e_conv2 = tf.compat.v1.layers.conv2d(
			# 		inputs=e_conv1,
			# 		filters=64,
			# 		kernel_size=[4, 4],
			# 		strides=(2, 2),
			...
			# 		kernel_initializer=w_initializer,
			# 		name='e_conv2')
			# e_conv3 = tf.compat.v1.layers.conv2d(
			# 		inputs=e_conv2,
			# 		filters=64,
			# 		kernel_size=[3, 3],
			# 		strides=(1, 1),
			...
			# e_pool = tf.compat.v1.layers.max_pooling2d(
			# 	inputs=e_conv1,
			# 	pool_size=[4, 4],
			# 	strides=(4, 4),
			# 	padding="same",
			# 	name='e_pool')
			#
			# e_flat = tf.compat.v1.layers.flatten(e_conv1)
			# e_dense1 = tf.compat.v1.layers.dense(
			# 		inputs=e_flat,
			# 		units=64,
			# 		activation=tf.nn.relu,
		...
		with tf.compat.v1.variable_scope('target_net'):
			target_net = tf.keras.applications.EfficientNetB0(
				include_top=False, weights=None, input_tensor=self.s,
				pooling="max"
			)
			# input_shape = tuple(list(self.s_.shape)[1:])
			target_net.trainable = True
			t_flat = tf.compat.v1.layers.flatten(target_net.output)
			...
			# 		kernel_initializer=w_initializer,
			# 		name='t_conv3')
			# t_pool = tf.compat.v1.layers.max_pooling3d(
			# 	inputs=t_conv1,
			# 	pool_size=[4, 4],
			# 	strides=(4, 4),
			# 	padding="same",
			# 	name='t_pool')
			# t_flat = tf.compat.v1.layers.flatten(t_conv1)
			# t_dense1 = tf.compat.v1.layers.dense(
			# 		inputs=t_flat,
			# 		units=64,
			# 		activation=tf.nn.relu,
