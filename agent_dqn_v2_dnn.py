...
class Agent_DQN(object):
    def __init__(self, env, args):
        ...
        self.epsilon_decrement = (self.epsilon - self.epsilon_min) / (
            100000)  # (x) -> x steps before epsilon reaches min
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope('eval_net'):
            ...
            e_pool = tf.compat.v1.layers.max_pooling2d(
                ...
                padding="same",
                name='e_pool')
            e_flat = tf.compat.v1.layers.flatten(e_pool)
        ...
        with tf.compat.v1.variable_scope('target_net'):
            ...
            #         kernel_initializer=w_initializer,
            #         name='t_conv1')
            t_pool = tf.compat.v1.layers.max_pooling2d(
                inputs=self.s,
                pool_size=[4, 4],
                strides=(1, 1),
                padding="same",
                name='t_pool')
            t_flat = tf.compat.v1.layers.flatten(t_pool)
        ...
        with tf.compat.v1.variable_scope('loss'):
            self.loss = tf.reduce_mean(
                input_tensor=tf.math.squared_difference(self.q_target, self.q_new, name='TD_error'))
