...
class Agent_DQN(object):
	...
	def build_model(self):
		...
		with tf.compat.v1.variable_scope('eval_net'):
			print('self s shape')
			print(self.s.shape)
			eval_net = tf.keras.applications.MobileNetV2(
				include_top=False, weights='imagenet', input_tensor=self.s,
				pooling="max"
			)
			eval_net.summary()
			eval_net.trainable = False
		...
		with tf.compat.v1.variable_scope('target_net'):
			target_net = tf.keras.applications.MobileNetV2(
				include_top=False, weights='imagenet', input_tensor=self.s,
				pooling="max"
			)
