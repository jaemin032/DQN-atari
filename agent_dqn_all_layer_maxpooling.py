...
class Agent_DQN(object):
	def __init__(self, env, args):
		...
		self.epsilon = 1.0
		# self.epsilon_decrement = (self.epsilon - self.epsilon_min) / (100000) # (x) -> x steps before epsilon reaches min
		self.epsilon_decrement = (self.epsilon - self.epsilon_min) / (1000000)  # (x) -> x steps before epsilon reaches min

		self.optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.99)
	...
	def get_path(self):
		...
		########################
		directory = './4_frame_all_maxpooling_model_new'
		try:
	...
	def build_model(self):
		...
		#--eval net--#
		self.s = tf.compat.v1.placeholder(tf.float32, n_features_tensor, name='s')  # input State
		#--target net--#
		self.s_ = tf.compat.v1.placeholder(tf.float32, n_features_tensor, name='s_')  # input Next State
		#--for loss--#
...
# 			eval_net = tf.keras.applications.MobileNet(
# 				include_top=True, weights=None, input_tensor=self.s,
# 				pooling=None, classes=self.n_actions, classifier_activation=None 
# 			)
# 			eval_net.trainable = True
# 			self.q_new = eval_net.output

			self.e_conv1 = tf.compat.v1.layers.conv2d(
					inputs=self.s,
					filters=32,
					kernel_size=[8, 8],
					strides=(2, 2),
					...
					name='e_conv1')
			self.e_maxpool1 = tf.compat.v1.layers.max_pooling2d(
                    inputs = self.e_conv1, pool_size=(2, 2), strides=(2,2), padding='valid', data_format=None)
			self.e_conv2 = tf.compat.v1.layers.conv2d(
					inputs=self.e_maxpool1,
					filters=64,
					kernel_size=[4, 4],
					strides=(1, 1),
					padding="same",
					...
					kernel_initializer=w_initializer,
					name='e_conv2')
			self.e_maxpool2 = tf.compat.v1.layers.max_pooling2d(
                    inputs = self.e_conv2, pool_size=(2, 2), strides=(2,2), padding='valid', data_format=None)
			self.e_conv3 = tf.compat.v1.layers.conv2d(
					inputs=self.e_maxpool2,
					filters=64,
					kernel_size=[3, 3],
					strides=(1, 1),
					padding="same",
					...
					kernel_initializer=w_initializer,
					name='e_conv3')
			self.e_maxpool3 = tf.compat.v1.layers.max_pooling2d(
                    inputs = self.e_conv3, pool_size=(2, 2), strides=(2,2), padding='valid', data_format=None)
			self.e_flat = tf.compat.v1.layers.flatten(self.e_maxpool3)
			self.e_dense1 = tf.compat.v1.layers.dense(
					inputs=self.e_flat,
					units=512,
					...
					name='e_dense1')
			self.q_new = tf.compat.v1.layers.dense(
					inputs=self.e_dense1,
...
# 			target_net = tf.keras.applications.MobileNet(
# 				include_top=True, weights=None, input_tensor=self.s_,
# 				pooling=None, classes=self.n_actions, classifier_activation=None 
# 			)
# 			# input_shape = tuple(list(self.s_.shape)[1:])
# 			target_net.trainable = True
# 			self.q_old = target_net.output
			
			t_conv1 = tf.compat.v1.layers.conv2d(
					inputs=self.s_,
					filters=32,
					kernel_size=[8, 8],
					strides=(2, 2),
					...
					kernel_initializer=w_initializer,
					name='t_conv1')
			t_maxpool1 = tf.compat.v1.layers.max_pooling2d(
                    inputs = t_conv1, pool_size=(2, 2), strides=(2,2), padding='valid', data_format=None)
			t_conv2 = tf.compat.v1.layers.conv2d(
					inputs=t_maxpool1,
					filters=64,
					kernel_size=[4, 4],
					strides=(1, 1),
					...
					kernel_initializer=w_initializer,
					name='t_conv2')
			t_maxpool2 = tf.compat.v1.layers.max_pooling2d(
                    inputs = t_conv2, pool_size=(2, 2), strides=(2,2), padding='valid', data_format=None)
			t_conv3 = tf.compat.v1.layers.conv2d(
					inputs=t_maxpool2,
					filters=64,
					kernel_size=[3, 3],
					strides=(1, 1),
					...
					kernel_initializer=w_initializer,
					name='t_conv3')
			t_maxpool3 = tf.compat.v1.layers.max_pooling2d(
                    inputs = t_conv3, pool_size=(2, 2), strides=(2,2), padding='valid', data_format=None)
			t_flat = tf.compat.v1.layers.flatten(t_maxpool3)
			t_dense1 = tf.compat.v1.layers.dense(
					inputs=t_flat,
					units=512,
					activation=tf.nn.relu,
					kernel_initializer=w_initializer,
					bias_initializer=b_initializer,
					name='t_dense1')
			self.q_old = tf.compat.v1.layers.dense(
					inputs=t_dense1,
					units=self.n_actions,
					activation=None,
					kernel_initializer=w_initializer,
		...
		with tf.compat.v1.variable_scope('loss'):
			self.loss = tf.reduce_mean(input_tensor=tf.math.squared_difference(self.q_target, self.q_new, name='TD_error'))
